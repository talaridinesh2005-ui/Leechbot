...
from .modules import authorize, clone, gd_count, gd_delete, gd_list, cancel_mirror, mirror_leech, status, torrent_search, torrent_select, ytdlp, \
                     ...
                     gd_clean, broadcast, category_select, premium, media_system
...
async def main():
    # Premium Startup Check
    try:
        if not plans:
            raise Exception("Premium plans not defined!")
        LOGGER.info("Premium System Active")
        await bot.send_message(OWNER_ID, f"✅ Bot Started\nPremium System Active\nTime: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    except Exception as e:
        LOGGER.error(f"Premium System Error: {e}")
        await bot.send_message(OWNER_ID, f"❌ Premium System Error:\n<code>{e}</code>")
