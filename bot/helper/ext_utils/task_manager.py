...
async def is_queued(uid, user_id=None):
    ...
    if user_id:
        ...
        is_priority = access.get('priority_queue', False)
        ...
        async with queue_dict_lock:
            ...
            if (all_limit and dl + up >= all_limit and (not dl_limit or dl >= dl_limit)) or (dl_limit and dl >= dl_limit):
                ...
                if is_priority:
                    ...
                    new_queued_dl = {uid: event}
                    new_queued_dl.update(queued_dl)
                    queued_dl.clear()
                    queued_dl.update(new_queued_dl)
                else:
                    queued_dl[uid] = event
    return added_to_queue, event


async def is_queued_up(uid, user_id=None):
    all_limit = config_dict['QUEUE_ALL']
    ...
    if all_limit or up_limit:
        async with queue_dict_lock:
            ...
            if (all_limit and dl + up >= all_limit and (not up_limit or up >= up_limit)) or (up_limit and up >= up_limit):
                ...
                if is_priority:
                    # Add to front of the queue
                    new_queued_up = {uid: event}
                    new_queued_up.update(queued_up)
                    queued_up.clear()
                    queued_up.update(new_queued_up)
                else:
                    queued_up[uid] = event
    return added_to_queue, event
...
async def limit_checker(size, listener, isTorrent=False, isMega=False, isDriveLink=False, isYtdlp=False, isPlayList=None):
    ...
    access = await check_user_access(user_id)
    if access['unlimited'] or await CustomFilters.sudo('', listener.message):
        return
    limit_exceeded = ''
    if listener.isClone:
...
async def task_utils(message):
    ...
    access = await check_user_access(user_id)
    if access['unlimited'] or await CustomFilters.sudo('', message):
        return msg, button
