...
class DbManger:
    ...
    async def update_premium_data(self, user_id, data):
        if self.__err:
            return
        await self.__db.premium.update_one({'user_id': user_id}, {'$set': data}, upsert=True)
        self.__conn.close

    async def get_premium_data(self, user_id):
        if self.__err:
            return None
    ...
    async def get_all_premium_data(self):
        if self.__err:
            return []
        return await self.__db.premium.find({}).to_list(length=None)

    async def update_user_stats(self, user_id, data):
        if self.__err:
            return
        await self.__db.users_stats.update_one({'user_id': user_id}, {'$set': data}, upsert=True)
        self.__conn.close

    async def get_user_stats(self, user_id):
        if self.__err:
            return None
        return await self.__db.users_stats.find_one({'user_id': user_id})

    async def get_transaction(self, transaction_id):
        if self.__err:
            return None
        return await self.__db.transactions.find_one({'_id': transaction_id})

    async def add_transaction(self, transaction_id, data):
        if self.__err:
            return
        await self.__db.transactions.update_one({'_id': transaction_id}, {'$set': data}, upsert=True)
        self.__conn.close
