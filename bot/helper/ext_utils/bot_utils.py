...
from system.access import check_user_access
...
async def get_user_stats_msg(user_id):
    access = await check_user_access(user_id)
    user_stats = await sync_to_async(DbManger().get_user_data, user_id) # Using existing user_data if loaded
    # Wait, user_data is global
    u_data = user_data.get(user_id, {})
    dly_tasks = u_data.get('dly_tasks', [datetime.now(), 0, 0, 0])

    premium_record = await sync_to_async(DbManger().get_premium_data, user_id)
    plan_name = premium_record.get('plan', 'Free') if premium_record else 'Free'
    expiry = premium_record.get('expiry_date', 'N/A') if premium_record else 'N/A'
    if expiry != 'N/A':
        expiry = expiry.strftime('%Y-%m-%d %H:%M:%S')

    daily_limit = config_dict.get('DAILY_TASK_LIMIT', 2) or 2
    daily_used = dly_tasks[1]

    # Queue Position
    queue_pos = "N/A"
    async with queue_dict_lock:
        all_queued = list(queued_dl.keys()) + list(queued_up.keys())
        # This is a bit simplified, but gives an idea
        user_tasks = [dl for dl in download_dict.values() if dl.message.from_user.id == user_id]
        if user_tasks:
            # Check if any task is in queue
            for task in user_tasks:
                if task.uid in all_queued:
                    queue_pos = all_queued.index(task.uid) + 1
                    break

    msg = f"📊 <b>User Statistics Panel</b>\n\n"
    msg += f"👤 <b>User ID:</b> <code>{user_id}</code>\n"
    msg += f"💎 <b>Plan:</b> {plan_name.capitalize()}\n"
    msg += f"⏳ <b>Expiry:</b> {expiry}\n"
    msg += f"📈 <b>Daily Used:</b> {daily_used} / {daily_limit if not access['unlimited'] else '∞'}\n"
    msg += f"🔢 <b>Queue Position:</b> {queue_pos}\n"
    msg += f"🌟 <b>Premium Status:</b> {'✅ Active' if access['is_premium'] else '❌ Inactive'}"
